...
const storyVoiceRes = (fileName) => `res://data/audio/story_content/${fileName}`;
...
function generate(model, dataRoot, opts = {}) {
  ...
  writePackManifest(P, model, chapters, flat);
  ensureSupportFiles(P);

  return { chapters: chapters.length, levels: flat.length, shards: shards.length };
}

// Write a manifest listing everything this pack owns, so the pack can be applied
// as an overlay and (optionally) validated. App-level files are never listed.
function writePackManifest(P, model, chapters, flat) {
  const doc = {
    pack_id: PACK_ID,
    name: (model.meta && model.meta.title) || "Isabella's Journey",
    description: (model.meta && model.meta.description) || '',
    version: (model.meta && model.meta.version) || '1.0.0',
    generated_at: new Date().toISOString(),
    chapters: chapters.length,
    levels: flat.length,
    // Paths (relative to data/) this pack owns. Anything not here is app-level
    // and must be preserved when applying the pack.
    owns: [
...
function paths(dataRoot) {
  return {
    ...
    // Narrative translations live under the content pack (NOT translations/core,
    // which holds stable app-level UI strings).
    packRoot: path.join(dataRoot, 'content_packs', PACK_ID),
    packTranslations: path.join(dataRoot, 'content_packs', PACK_ID, 'translations'),
    packManifest: path.join(dataRoot, 'content_packs', PACK_ID, 'pack.json'),
...
function writeNarrativeStages(P, flat) {
  flat.forEach((entry) => {
    ...
    const states = dialogue.map((d, di) => {
      ...
      if (d.voice) st.voice = storyVoiceRes(d.voice);
    ...
    // The last line has NO outbound transition on purpose: the engine treats a
    // state with no outbound transition as terminal and completes the stage
    // after its `duration`. (See NarrativeStageController._set_state.)
...
function writeTranslations(P, poSourceRoot, flat) {
  // Narrative translations are pack-scoped and contain ONLY NARRATIVE_* keys.
  // They are written to content_packs/<PACK_ID>/translations/narrative_<lang>.po
  // and NEVER to translations/core (which holds stable app-level UI strings).
  const srcPack = path.join(poSourceRoot, 'content_packs', PACK_ID, 'translations');
  ensureDir(P.packTranslations);

  const langName = { en: 'English', es: 'Spanish', pt: 'Portuguese', fr: 'French' };

  for (const lang of LANGS) {
    // Preserve existing per-locale translations for keys we regenerate (so
    // human translations aren't lost on round-trip).
    const existing = readPo(path.join(srcPack, `narrative_${lang}.po`));
    ...
    flat.forEach((entry) => {
      ...
      dialogue.forEach((d, di) => {
        ...
        entries.push({ id: key, str: val });
      });
    });

    writePo(path.join(P.packTranslations, `narrative_${lang}.po`), langName[lang], lang, entries);
  }
}

// Support files — only create if genuinely missing; never clobber tuned ones.
// NOTE: reward container/theme configs (theme_container_mappings.json,
// container_selection_rules.json, reward_containers/, reward_profiles/) are
// APP-LEVEL and are intentionally NOT written here — a content pack must never
// create or reset them.
function ensureSupportFiles(P) {
  // The flow references definition_id "narrative_stage"; provide the shared
  // step defaults only if the target project doesn't already have them.
