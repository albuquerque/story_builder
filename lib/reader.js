...
const { PATHS, FLOW_ID, COLLECTION_ID, PACK_ID } = require('./config');
...
function readProject() {
  ...
  // Narrative text lives in the content pack. Fall back to the legacy
  // translations/core location for older projects that haven't migrated yet.
  const packNarrativeEn = path.join(PATHS.contentPackRoot, PACK_ID, 'translations', 'narrative_en.po');
  const legacyCoreEn = path.join(PATHS.translationsCore, 'strings_en.po');
  const en = fs.existsSync(packNarrativeEn) ? readPo(packNarrativeEn) : readPo(legacyCoreEn);
  ...
  if (stage) {
    ...
    c.dialogue = states.map((s) => {
      ...
      return {
        ...
        // Per-screen voiceover / narration clip (basename).
        voice: s.voice ? basenameFromRes(s.voice) : '',
